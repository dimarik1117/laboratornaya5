...
def registration():
    if request.method == 'POST':
        ...
        if not name or not login or not password:
            return render_template('registration.html', error="There should be no empty inputs.")

        if name == login or name == password or login == password:
            return render_template('registration.html', error="There should not be identical inputs.")
